...
def find_all(service: SugestaoService = Depends(get_service)):
    return service.find_all()

@router.get("/{id}", response_model=SugestaoResponse)
def find_by_id(id: int, service: SugestaoService = Depends(get_service)):
    return service.find_by_id(id)

@router.post("/create", response_model=SugestaoResponse)
def create(request: SugestaoRequest, service: SugestaoService = Depends(get_service)):
...
@router.delete("/{id}")
def delete(id: int, service: SugestaoService = Depends(get_service)):
    service.delete(id)
    return {"message": "Sugestao deletada com sucesso"}

@router.patch("/{id}/status")
def update_status(id: int, status: str, service: SugestaoService = Depends(get_service)):
